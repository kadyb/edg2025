...
Dokonaj eksploracyjnej analizy danych dla zbioru zamieszczonego w katalogu `dane`.
Wykorzystaj poznane na zajęciach techniki wizualne oraz opisowe, aby sprawdzić:
...
Obliczenia wykonaj na podstawie losowej próby. Pamiętaj, aby ustawić ziarno
losowości! Do każdego etapu analizy obowiązkowo dołącz wnioski.

Jako rozwiązanie należy przesłać:

1. Działający plik źródłowy (np. plik Quarto `.qmd` lub RMarkdown `.Rmd`). 
