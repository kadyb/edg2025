...
- Rozkład skośny -- rozkład asymetryczny z większością wartości skupioną po
...
zbliżonym do normalnego. Zasadniczo, test Kołmogorowa-Smirnowa ma niższą moc
statystyczną niż test Shapiro-Wilka, jednak można stosować go do większych prób
niż 5000 obserwacji.
...
niekoniecznie liniową (test Spearmana i Kendalla). Testy nieparametryczne mają
mniejszą moc statystyczną, jednak są bardziej uniwersalne, gdy założenia testów
parametrycznych nie są spełnione.

Test Pearsona stosuje się do zmiennych ilościowych, natomiast testy Spearmana
i Kendalla do zmiennych ilościowych oraz do zmiennych jakościowych na skali
porządkowej. Żaden z tych testów nie jest przeznaczony do zmiennych jakościowych
na skali nominalnej (np. klasy pokrycia terenu).
...
Dokonaj eksploracyjnej analizy danych dla plików rastrowych (`*.tif`)
zamieszczonych w katalogu `dane`. Zaprezentuj wczytane zmienne na mapach,
a następnie wykorzystaj poznane na zajęciach techniki wizualne oraz opisowe,
aby sprawdzić:
...
Obliczenia wykonaj na podstawie losowej próby i napisz jaka wielkość próby
została wykorzystana. Pamiętaj, aby ustawić ziarno losowości! Do każdego etapu
analizy obowiązkowo dołącz komentarz z wnioskami.
