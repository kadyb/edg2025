...
używany zaróno w środowisku akademickim, jak i w przemyśle, szczególnie w takich
...
Jeśli mamy do czynienia ze strukturą zagnieżdzoną, to do selekcji używamy kolejne
...
Przeznaczeniem eksploracji jest znalezenie odpowiedzi na następujące pytania:
...
4. Czy wsytępuje jakaś zależność (korelacja) pomiędzy zmiennymi?

## Opisowa

- `str()` -- wyświetla strukturę obiektu, obejmującą nazwy kolumn, typy danych
i przykładowe wartości.
- `summary()` -- zwraca statystyki opisowe.
...
Histogram również służy do przedstawienia rozkładu danych liczbowych jednak
w postaci słupkowej. Pokazuje jak często dane wartości (lub zakresy) występują
w danych.
...
$\pi$ użyj stałej `pi`.
2. Definiując dwie zmienne `imie` oraz `wiek`, wykorzystaj funkcję `paste()`
do stworzenia tekstu o następującej treści:
`Cześć! Moje imię to <imie>. Mam <wiek> lat.`
3. Używając funkcji `seq()`, wygeneruj wektor (sekewncje) liczb parzystych od
2 do 20 i następnie oblicz sumę, średnią oraz odchylenie standardowe jego
elementów używając odpowiednich funkcji.
4. Dokonaj eksploracji opisowej dostępnego zbioru danych `iris`.
