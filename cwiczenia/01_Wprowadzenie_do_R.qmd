...
**Uwaga!** Obliczanie statystyk opisowych (takich jak średnia, mediana czy
odchylenie standardowe) dla zmiennej jakościowej (np. rodzaj skrzyni biegów,
klasy pokrycia terenu) nie ma sensu, ponieważ są to właściwie tylko etykiety
bez znaczenia numerycznego. W przypadku zmiennych jakościowych (kategorycznych)
można wyznaczyć modę (dominantę) czy częstość występowania poszczególnych
kategorii. Aby środowisko R poprawnie traktowało te zmienne jako kategorie
(grupy), należy dokonać konwersji z typu numerycznego na czynnikowy, używając
funkcji `as.factor()`.
...
4. Dokonaj eksploracyjnej analizy wbudowanego zbioru danych `iris`. W szczególności
sprawdź strukturę danych, typy zmiennych, liczbę kolumn i wierszy, unikalne
gatunki oraz statystyki opisowe.
...
`Species`) oraz dodaj legendę do wykresu. Wykres musi posiadać wszystkie niezbędne
elementy do jego poprawnej interpretacji.
