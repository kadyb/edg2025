...
funkcje `which()` z argumentem `arr.ind = TRUE`. Zastanów się również jak na
wyniki analizy wpływają miasta na prawach powiatu oraz
[eksklawy powiatów](https://pl.wikipedia.org/wiki/Eksklawa#Powiaty).

5. W katalogu `dane` znajdziesz 9 warstw rastrowych. Sprawdź ich metadane.
Jeśli wszystkie parametry rastrów są identyczne, to połącz warstwy reprezentujące
zmienne ilościowe w jeden raster wielokanałowy i zapisz go na dysku.
