...
Wariancja jest to miara statystyczna, która opisuje ilościowo zróżnicowanie
(rozrzut) wartości w zbiorze danych, określając jak bardzo odbiegają od wartości
średniej tego zbioru. Wysoka wariancja wskazuje, że punkty są rozproszone,
podczas gdy niska wariancja oznacza, że są one skupione blisko średniej.
Zerowa wariancja oznacza, że wszystkie wartości są identyczne (brak zmienności).
Należy pamiętać, że wariancja wyrażona jest w jednostkach kwadratowych ($j^2$)
...
W analizie danych wariancja traktowana jest jako miara ilości informacji.
Przyjmuje się, że im większa zmienność danej cechy, tym więcej istotnych
informacji ona niesie. W rzeczywistych zbiorach danych często spotykamy się
z ogromną liczbą cech (zmiennych), które opisują pewne obserwacje (obiekty).
Jest to tak zwane zjawisko wielowymiarowości danych (*high dimensionality*).
W praktyce, powoduje to problemy związane z ilością wymaganej pamięci do
przetwarzania danych, dłuższym czasem obliczeń, trudnościami w interpretacji
zależności i ich wizualizacją w przestrzeni dwuwymiarowej, a także zmniejszenie
skuteczności modelu wraz ze wzrostem liczby zmiennych (słabsza generalizacja
zależności).

Rozwiązaniem tych problemów może być zastosowanie **analizy głównych składowych**
(*principal component analysis*, PCA), która jest najczęściej stosowaną
metodą do redukcji wymiarowości. Przekształca ona zbiór danych ze
skorelowanymi zmiennymi w nowy zbiór nieskorelowanych (ortogonalnych) zmiennych
nazywanymi głównymi składowymi (*principal components*), zachowując przy tym
jak najwięcej zmienności (informacji). Główne składowe są liniowymi kombinacjami
oryginalnych zmiennych, uporządkowanymi według ilości wariancji, którą wyjaśniają.
Oznacza to, że pierwsza główna składowa (PC1) wyjaśnia największą zmienność
(wariancję), a każda kolejna składowa (PC2, PC3, itd.) mniej. Jeśli główna
składowa nie wykazuje prawie żadnej wariancji, to ma ona niewielki wkład w
reprezentację zbioru danych i z tego powodu jest wykluczana z dalszej analizy.
...
2. Dane są typu ilościowego (dane jakościowe wymagają odpowiedniego kodowania).
3. Wszystkie obserwacje mają wartości (brak `NA` w zbiorze danych).
4. Ograniczenie wpływu wartości odstających, ponieważ zniekształcają wyniki.
5. Zmienne muszą posiadać jednakowe skale (wymagana standaryzacja danych).
...
oparte na sieciach neuronowych czy techniki służące wyłącznie do wizualizacji,
takie jak *t-SNE* i *UMAP*.
...
używając analizy głównych składowych (*PCA*) i losowej próby. Przed analizą
przygotuj odpowiednio dane wejściowe:

- usuń brakujące wartości (`NA`),
- zidentyfikuj i usuń wartości odstające,
- jeśli rozkład zmiennych istotnie odbiega od normalnego, to spróbuj zastosować
transformacje,
- pamiętaj o standaryzacji zmiennych.

Finalnym wynikiem analizy jest mapa przedstawiająca pierwszą główną składową
wraz z interpretacją, co ona przedstawia. W raporcie należy wyjaśnić i uzasadnić
wybór zastosowanych metod analitycznych.
