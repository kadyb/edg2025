...
Pamiętaj, że wariancja jest wyrażona w kwadratowych jednostkach ($j^2$)
...
Następnie dane z próby należy ustandaryzować do wspólnej skali przy użyciu funkcji
...
polegającego na ekstrapolacji wyliczonych główych składowych z próby na cały
...
(`SpatRaster`) do ramki danych. Ustawmy także argumeny `na.rm = FALSE` do
zachowania pustych wartości w rastrze. Następnie, kluczowe krokiem jest wyznaczenie
indeksów komórek, które posiadają wartości używając funkcji `complete.cases()`,
ponieważ będzie to niezbędne do wstawienia wartości w odpowiednie miejsca w
wektorze.
...
indeksów z obiektu `idx` przypiszmy wartości pierwszej głównej składowej do
